...
local Window = Fluent:CreateWindow({
    Title = "Renren Hub 💫",
    SubTitle = "Script By Renren",
    TabWidth = 160,
    Size = UDim2.fromOffset(500, 320),
    Acrylic = false, 
    Theme = "Darker",
    MinimizeKey = Enum.KeyCode.LeftControl -- Nhấn Ctrl Trái để Ẩn/Hiện Menu
})
...
-- Thêm các nút Bật/Tắt (Toggle) vào Tab Main
local ToggleESP = Tabs.Main:AddToggle("ToggleESP", {Title = "ESP Item (Nhìn xuyên tường)", Default = false })
ToggleESP:OnChanged(function(Value)
    setESPMode(Value)
end)

local ToggleAura = Tabs.Main:AddToggle("ToggleAura", {Title = "Aura (Tự động nhặt/mở)", Default = false })
ToggleAura:OnChanged(function(Value)
    setAuraMode(Value)
end)

local ToggleSpeed = Tabs.Main:AddToggle("ToggleSpeed", {Title = "Chạy Nhanh (Speed 32)", Default = false })
ToggleSpeed:OnChanged(function(Value)
    setSpeedMode(Value)
end)

local ToggleNoclip = Tabs.Main:AddToggle("ToggleNoclip", {Title = "Đi Xuyên Tường (Noclip)", Default = false })
...
Tabs.Info:AddParagraph({
    ...
    Content = "ESP Chams Rainbow: PetaPeta, EnemyModel/EnemyModels (Luôn bật)\nESP Item: Box, Safe, Key, dll (Bật/Tắt)\nAura: Tự động Search/Pick Up/Open\nSpeed: Loop 32 WalkSpeed\nNoclip: Đi xuyên tường, chống rớt map\nBy: Renren Hub 💫"
})

-- Mở sẵn tab Main khi mới chạy
Window:SelectTab(1)

-- Hiển thị thông báo khi load thành công
Fluent:Notify({
    Title = "Thành công!",
    Content = "Renren Hub đã load xong giao diện mới.",
    Duration = 5
})